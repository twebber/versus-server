...
var checkForEndGame = function(game) {
  var numberOfQuestions = game.questions.length
    , numberOfQuestionsWithCorrectChoices = _(game.questions).filter(function(question) {
      return _(question.choices).find(function(choice) {
        return choice.isCorrect;
      });
    }).length;

  game.endedAt = numberOfQuestionsWithCorrectChoices === numberOfQuestions ? new Date : null;

  return Game.clean(game);
};

Updater.prototype.update = function(id, game) {
  var finder = new Finder(this.user, this.app);

  game = checkForEndGame(game);

  return Game.update(id, game).then(function() {
